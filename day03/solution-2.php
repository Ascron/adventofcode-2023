...
function solution (array $input) {
        ...
        foreach ($digitLine as $x => $digit) {
            ...
            foreach ($possibleYs as $possibleY) {
                foreach ($possibleXs as $possibleX) {
                    if (isset($symbols[$possibleY][$possibleX]) && $symbols[$possibleY][$possibleX] === '*') {
                        $gears[$possibleY][$possibleX] ??= [];
                        $gears[$possibleY][$possibleX][] = $digit;
                        break 2;
                    }
                }
            }
    ...
    foreach ($gears as $gearLine) {
        foreach ($gearLine as $gear) {
            if (count($gear) === 2) {
                $result += array_product($gear);
            }
        }
    }
